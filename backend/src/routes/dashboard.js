...
router.get(
  "/",
  protect,
  cacheMiddleware({ expireSeconds: 120 }),
  async (req, res) => {
    try {
      ...
      const user = await User.findById(userId).select(
        "name email plan planExpiresAt stats settings createdAt",
      );
      ...
      res.json({
        ...
        dashboard: {
          ...
          stats: {
            ...
            avgConceptsPerGraph: Math.round(
              aggregateStats[0]?.avgConcepts || 0,
            ),
            avgRelationshipsPerGraph: Math.round(
              aggregateStats[0]?.avgRelationships || 0,
            ),
