...
function Dashboard({ onNavigate, onLoadGraph }) {
  ...
  if (error) {
    return (
      ...
      >
        <div
          className={`text-center p-8 rounded-2xl border ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}
        >
          ...
          <div>
            ...
            >
              ...
              <div className="space-y-2">
                {stats?.graphsByType &&
                  Object.entries(stats.graphsByType).map(([type, count]) => (
                    <div
                      key={type}
                      className="flex items-center justify-between"
                    >
                      <span
                        className={`text-sm capitalize ${isDark ? "text-gray-400" : "text-gray-600"}`}
                      >
                        {type}
                      </span>
                      <span
                ...
                {(!stats?.graphsByType ||
                  Object.keys(stats.graphsByType).length === 0) && (
                  <p
