...
import { User, LogOut, History, Settings, ChevronDown, LayoutDashboard, FolderOpen, UserCircle } from 'lucide-react';
        ...
        <div className={`absolute right-0 top-full mt-2 w-64 border rounded-xl shadow-2xl overflow-hidden z-50
          ...
          <div className="py-2">
            {menuItems.map((item) => (
              ...
              >
                <item.icon className={`w-4 h-4 ${isDark ? 'text-dark-400' : 'text-dark-500'}`} />
                <span className={isDark ? 'text-dark-200' : 'text-dark-700'}>{item.label}</span>
              </Link>
            ))}
          </div>
