    ...
    >
      <div className="flex items-start gap-3">
        <div className={`p-2 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-100'}`}>
          <SourceIcon className={`w-5 h-5 ${isDark ? 'text-gray-400' : 'text-gray-500'}`} />
        </div>
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
            {graph.conceptCount} concepts · {graph.relationshipCount} relationships
          </p>
        </div>
        <ChevronRight className={`w-5 h-5 opacity-0 group-hover:opacity-100 transition-opacity ${isDark ? 'text-gray-400' : 'text-gray-500'}`} />
      </div>
    </button>
  );
};
...
const QuickAction = ({ icon: Icon, label, description, to, gradient }) => {
  ...
  return (
    ...
    >
      <div className={`inline-flex p-3 rounded-xl ${gradient} mb-3 group-hover:scale-110 transition-transform`}>
        <Icon className="w-6 h-6 text-white" />
      </div>
          ...
          <div className="lg:col-span-2">
            ...
            {recentGraphs.length > 0 ? (
              ...
              <div className={`p-8 rounded-2xl border text-center
                ...
                <Network className={`w-12 h-12 mx-auto mb-4 ${isDark ? 'text-gray-600' : 'text-gray-400'}`} />
